...
if __name__ == '__main__':
    ...
    X,y,first_points = pipeline(df_train,args.size,verbose=True)
    ...
    train_dataloader = DataLoader(TrajectoryDataset(X[:round(size*0.95)],y[:round(size*0.95)],first_points[:round(size*0.95)],56960,test=False,head=args.head,random=args.random_length),300,True,drop_last=True)
    test_data = TrajectoryDataset(X[round(size*0.95):],y[round(size*0.95):],first_points[round(size*0.95):],56960,test=True,head=args.head,random=args.random_length)
    test_dataloader = DataLoader(test_data,len(test_data))
    print("[INFO] data load finish")
    X_test,y_test,first_points_test = None,None,None
    for x,y,first_points in test_dataloader:
        X_test = x.to(device)
        y_test = y.to(device)
    ...
    for epoch in range(args.epoch):
        ...
        model.train()
        loss_sum = 0
        for X,y,first_points in tbar:
            pred = model(X.to(device),first_points.to(device))
        ...
        model.eval()
        pred = model(X_test,first_points_test).detach()
